...
export class Account {
    ...
    async getAccountByPuuid(puuid: string): Promise<AccountDto | null> {
        try {
            return await this.client.get(`/riot/account/v1/accounts/by-puuid/${puuid}`);
        } catch (error) {
            console.error(error);
        }
        return null;
    ...
    async getAccountByRiotId(gameName: string, tagLine: string): Promise<AccountDto | null> {
        try {
            return await this.client.get(`/riot/account/v1/accounts/by-riot-id/${gameName}/${tagLine}`);
        } catch (error) {
            console.error(error);
        }
        return null;
    ...
    async getActiveRegionByPuuid(puuid: string): Promise<AccountRegionDto | null> {
        try {
            const game = 'lol';
            return await this.client.get(`/riot/account/v1/region/by-game/${game}/by-puuid/${puuid}`);
        } catch (error) {
            console.error(error);
        }
        return null;
